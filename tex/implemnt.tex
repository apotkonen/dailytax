...
Implementation is most likely embedded into existing systems besides operating
ones. So it would be possible to run new process besides old ones with correct
current data. Some code is already available as Open Source Software
(OSS)\cite{FSFE_PublicCode}\cite{DayTax}...
...
System hast to fixed or removed!...\cite{STM061_2025}.
...
technososioeconomic way optimized allocation and use.
...
Public money -- public code\cite{FSFE_PublicCode}, public service --
public data\cite{OpenPublicData}, public society -- public
processes\cite{VM_GBEA}\cite{Kukkonen2025},
public and by nature modular architecture are good values
to maintain cooperation and common wealth, because everybody's contribution
can multiply common output and this way bring civil society efforts to common
use, same time reducing overall cost and time what takes to get results and
control position for democratically selected government from society's own
...
and development is CRITICAL SERVICE, not a project\cite{VM_GBEA}. Architecture
all time evolving at least now when we live era of fast digitalization. We need
current state, target state, gap and next development step visibility online to
be up to date, and discussion from next steps ongoing for sub areas needing
development work. Modular architecture selection supports work division,
utilization of the others results, as well making possible procurement using
small vendors or consortium model where several small providers bring they
efforts to common pool.

It's really hard task to get needed human centric over the
...
practical day to day work. For work we really need public version from
government enterprice architecture\cite{Kukkonen2025} which shows at least
processes over the whole governmental field. Created new digitalized processes
has to be tecnososioeconomical optimized versions before implementation
procurement.
